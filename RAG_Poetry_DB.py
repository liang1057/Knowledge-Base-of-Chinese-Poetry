...
if __name__ == '__main__':
    ...
    # 加载诗词（从json）
    author_drop = []
    all_poems = load_all_poems(filename='./data/Poetry_China_all.json')
    print("收录诗词的朝代")
    ...
    dynasty_dict = {}
    for i, d in enumerate(json_dynasty):
        dynasty_dict[d['name']] = d['dynasty_id']
        dynasty_dict[d['dynasty_id']] = d['name']
    ...
    for i, dynasty in enumerate(all_poems.keys()):
        ...
        for j, author in enumerate(all_poems[dynasty].keys()):
            ...
            for k, poem in enumerate(poem_list):
                ...
                try:
                    # 插入诗词
                    entity_poem.SetValue('poem_id', f'{author_id}_{k:05d}')
